...
# Modified in 2026 to expose native BTX adoption to EXL3 adapters.
...
from ._btx_adoption import adopt_prepared_btx_weights as adopt_btx_weights
from ._impl import (
...
def plan_weights(**kwargs: Any) -> WeightPlan | WeightsPlan:
    ...
    if "quant_modes" in kwargs:
        # B12X 1.3 removed the explicit rate-structure field from its vLLM
        # compatibility plan.  Brandon's EXL3 adapter still supplies it for
        # the uniform-K4 checkpoint.  Preserve strictness for anything else,
        # then translate the old contract to the new one.
        trellis_rate_structure = kwargs.pop("trellis_rate_structure", None)
        if trellis_rate_structure not in (None, "uniform"):
            raise ValueError(
                "B12X 1.3 compatibility planning only supports uniform "
                f"Trellis rates, got {trellis_rate_structure!r}"
            )
